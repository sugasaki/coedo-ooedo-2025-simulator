...
describe('pathUtils', () => {
  ...
  describe('calculateCumulativeDistances', () => {
    ...
    it('複数点の累積距離を正しく計算する', () => {
      const coordinates = mockGeoJSON.features[0].geometry.coordinates as Coordinate3D[];
      const result = calculateCumulativeDistances(coordinates);
  ...
  describe('getPositionAtDistance', () => {
    ...
    it('距離が負の場合は始点の座標を返す', () => {
      const result: PositionAtDistance = getPositionAtDistance(mockGeoJSON, -10);
      expect(result).toEqual(mockGeoJSON.features[0].geometry.coordinates[0]);
    });

    it('距離がコース総距離を超える場合は終点の座標を返す', () => {
      const result: PositionAtDistance = getPositionAtDistance(mockGeoJSON, 10000);
